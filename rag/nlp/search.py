...
from rag.nlp import rag_tokenizer, es_queryer
import numpy as np
...
class Dealer:
    def __init__(self, es):
        self.qryr = es_queryer.EsQueryer(es)
        self.qryr.flds = [
            "title_tks^10",
            "title_sm_tks^5",
            "important_kwd^30",
    ...
    def search_chunk_list(self, req, idxnm, emb_mdl=None):
        ...
        aggs = self._getAggregation(res, "docnm_kwd")
        ...
        return self.SearchResult(
            ...
            query_vector=q_vec,
            aggregation=aggs,
            highlight=self._getHighlight(res),
            field=self._getFields(res, src),
            keywords=list(kwds)
        )

    def _getAggregation(self, res, g):
        if not "aggregations" in res or "aggs_" + g not in res["aggregations"]:
            return
        bkts = res["aggregations"]["aggs_" + g]["buckets"]
        return [(b["key"], b["doc_count"]) for b in bkts]

    def _getHighlight(self, res):
        def rmspace(line):
    ...
    def retrieval(self, question, embd_mdl, tenant_id, kb_ids, page, page_size, similarity_threshold=0.2,
        ...
        for i in idx:
            ...
            d = {
                ...
                "vector": self._trans2floats(sres.field[id].get("q_%d_vec" % dim, "\t".join(["0"] * dim))),
